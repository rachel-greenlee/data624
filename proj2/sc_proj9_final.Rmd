...
_Decision Tree_: can capture nonlinear relationships, little data preprocessing, doesnt require normalizing, easy to interpret, can get sense of feature importance, sensitive to collinearity as well as changes in data input, lower accuracy than other models.
...
Our data has already been split into a training and testing dataset. To use these data in a Tidymodels modeling process, we will combine the two as and reverse engineer our split.
...
# calcualte proportion of data from train
...
We start start model development by setting up our resampling scheme for parameter optimization and model selection. Here we will use 10-kfold Cross-validation and identify PH as our response variable. 
...
We now construct our initial model workflows and combine them using workflow_set() to streamline our training process. The workflows themselves, link each model's recipe and specification. 
...
From Table ? and Figure ?, it is clear that the MARS model outperformed the other models - i.e., lowest rmse & smallest std_err. 
...
rank_results(grid.results, rank_metric = "rmse")%>%
  flextable()%>%
  set_caption('Table ??. Model Performance Metrics for Grid Tuning')
...
autoplot(
  ...
  lims(y = c(0, 0.75)) +
  labs(title='Figure ?. Estimated RMSE for Best Model Configuration', x='')+
  theme_light()
```

We can also extract and compare the highest performing resample model for each algorithm. These estimates are included in Table ?. 
...
collect_metrics(grid.results)%>%
  ...
  select(!.metric)%>%
  flextable()%>%
  set_caption('Table ?. Selected Models Based on Training Fit')
```
# Model Evaluation
...
rbind(metrics1, metrics2, metrics3, metrics4, metrics5)%>%
   arrange(Estimate)%>%
   flextable()%>%
   set_caption('Table ?. Comparison of Model Fit on Test Data')

```
## Model Interpretation

One advantage of MARS models is their interpretability. Table ? shows the variables selected for the final model along with their respective coefficients. 
...
Figure ?. displays the partial dependence between PH and each of the predictors for the MARS model. In this context, each plot describes the effect of a predictor with all other covariates held at their average value. 
...
output[12]%>%
  ...
  flextable%>%
  set_caption('Table ?. Final MARS Model: Features & Coefficients')


# plot partial dependence of features (from plotmo package in Earth -- plot regression surfaces)

plotmo(output, pmethod='partdep', caption='FIgure ?. Partial dependence plots for continuous predictors using MARS')

```
To effective visualize the model results we can plot our observed PH values against the model PH predictions for both training and test sets. These results as shown in Figure ? and Table ?. Our errors are lowest around ~ PH = 8.5. Our model tends to overpredict at lower values and overpredict at higher values for PH on the test data. 
...
model.predictions %>%
  ...
  labs(x='PH (Observed)', y= 'PH (Predicted)', title='Figure ?. Observed vs Predicted PH Values: MARS Model')+
  theme(plot.title = element_text(vjust = 10))+ # this doesnt seem to be working
  theme_minimal() +
  facet_wrap(~type)
...
model.predictions %>%
  ...
  select(!mae)%>%
  flextable()%>%
  set_caption('Table ?. Performance Metrics on Final Training and Test Datasets: MARS')
  
  
  

```
Finally, we can rank the relative importance of each covariate in terms of their contribution as predictors of PH. From Table ?., we that 'MnF Flow' makes the greatest contribution to the model prediction. For this dataset, only four covariates were included in the final model. 
...
mars.test%>%
  ...
  labs(title='Figure ?. Variable Importance: MARS Model', subtitle='PH')+
  theme_light()
